...
These three datasets were merged using `left_join()` with `pols_month`
dataset as the reference. The merge was performed on the year and month
...
Gwynns Falls Trash Wheel dataset
...
merged_trash =
  bind_rows(
    ...
    gwynns_trash |>  mutate(wheel = "gwynns")
  ) |> 
...
The `merged_trash` dataset contains information on 1188 dumpsters
collected by three different wheels, covering the period from 2014 to
2025.

The `mr_trash` dataset has 707 dumpsters recorded with 14 different
...
The `Professor Trash Wheel` recorded 132 dumpsters and total weight of
trash collected was 282.26 tons.

The `Gwynns Falls Trash Wheel` collected 349 dumpsters data collection
and collected the total number of cigarette butts was 18120 in June of
2022.
...
always consistent. Therefore, in the merged dataset, I used only
zip_code as the unique join key between Zillow ZORI data and NYC ZIP
code data.

``` r
merge_zori = 
  left_join(zipcode, zillow_nyc, by = "zip_code", relationship = 'many-to-many') |> 
...
```
    ...
    ##  2 Bronx          36 005               36005    10451 7/25/07   High Bridge and…
    ##  3 Bronx          36 005               36005    10451 7/25/07   High Bridge and…
    ##  4 Bronx          36 005               36005    10451 7/25/07   High Bridge and…
    ##  5 Bronx          36 005               36005    10451 7/25/07   High Bridge and…
    ##  6 Bronx          36 005               36005    10451 7/25/07   High Bridge and…
    ##  7 Bronx          36 005               36005    10451 7/25/07   High Bridge and…
    ##  8 Bronx          36 005               36005    10451 7/25/07   High Bridge and…
    ##  9 Bronx          36 005               36005    10451 7/25/07   High Bridge and…
    ## 10 Bronx          36 005               36005    10451 7/25/07   High Bridge and…
    ## # ℹ 17,677 more rows
    ## # ℹ 10 more variables: region_id <dbl>, size_rank <dbl>, region_type <chr>,
    ## #   state_name <chr>, state <chr>, city <chr>, metro <chr>, county_name <chr>,
    ## #   date <chr>, zori <dbl>
...
details from the NYC Zip Code dataset. In total, it contains 17687
observations with 320 unique zip codes and 43 unique neighborhoods. Each
observation includes the state zip code, county, borough, and
...
``` r
not_included = 
  anti_join(zipcode, zillow_nyc, by = "zip_code") 
```
...
covid_19 = merge_zori |> 
  ...
  filter(
    date %in% c("2020_01_31", "2021_01_31")
  ) |> 
  pivot_wider(
    names_from = date,
    values_from = zori,
  ...
  mutate(
    drop = zori_2020_01_31 - zori_2021_01_31
  ) |> 
  arrange(desc(drop)) |> 
  slice(1:10) 
...
| zip_code | county | neighborhood | zori_2020_01_31 | zori_2021_01_31 | drop |
|---:|:---|:---|---:|---:|---:|
...
and Soho. This may be because mainly commuters and tourists are
concentrated in New York county but during the COVID-19 people can not
travel or commute and due to social distancing policies and remote
working. Therefore, rents in New York county dropped much than in other
counties.
